 ...
 *
 * The seeded PND 1888: weekday RM11 / off-peak RM13, daily deduction 2, only
...
describe("MONEY integration — finalize & day-ledger through Postgres", () => {
  ...
  it("per-zone-per-day: two same-zone trips pay full then repeat (RM44 + RM11 = RM55 on a weekday)", async () => {
    ...
    // Trip 1 — A2 (6pts), the day's first drop → full 6 minus PND deduction 2.
    const t1 = await bookTrip(requestor, ["A2"], rt);
    await approveTrip(admin, t1.id, driverId, PND_PLATE);
    ...
    await approveTrip(admin, t2.id, driverId, PND_PLATE);
  ...
  it("midnight straddle: one trip whose drops fall on two MYT days sums each day's marginal", async () => {
    ...
    // 2-stop trip: A2 (6pts) then A1 (5pts) — both covered by the primary.
  ...
  it("rate lock: an in-flight trip finalizes at its assignment-time rate, immune to a mid-day edit", async () => {
    ...
    const truckBefore = (await prisma.truck.findUnique({ where: { plate: PND_PLATE } }))!;
    ...
    const patch = await api()
      .patch(`/api/v1/trucks/${encodeURIComponent(PND_PLATE)}/rates`)
      .set(auth(admin))
      .send({ entitled_claim_weekday: preWeekday + 50, entitled_claim_offpeak: preOffpeak + 50 });
    expect(patch.status).toBe(200);
    const edited = (await prisma.truck.findUnique({ where: { plate: PND_PLATE } }))!;
