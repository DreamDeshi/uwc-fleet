// Minimal zero-dependency static file server for the Expo web export (dist/).
// Used as the Railway start command. Serves hashed assets directly and falls
// back to index.html for client-side routes (React Navigation). No deps so the
// production image stays lean and there's no ESM/CJS interop to trip over.
// Mirrors admin/serve.mjs.
...
}).listen(PORT, () => console.log(`UWC mobile web static server listening on :${PORT}`));
